/**
 * js/auth.js ÔÇö Configura├º├úo e L├│gica do Supabase (LIMPO E ORGANIZADO)
 */
...
if (window.supabase) {
  try {
    ...
    // ­ƒÜÇ Redirecionar se j├í estiver logado (evitar registro/login duplicado)
    (async () => {
      ...
      if (isAuthPage && !isRecovery) {
        // Se a pessoa atualizou a p├ígina no meio da recupera├º├úo (perdeu o hash), n├│s a desconectamos
        if (sessionStorage.getItem('is_recovering_password') === 'true') {
          sessionStorage.removeItem('is_recovering_password');
          await supaClient.auth.signOut();
          return; // Para aqui, for├ºando ela a ficar na p├ígina de login limpa
        }
    ...
    supaClient.auth.onAuthStateChange((event, session) => {
      console.log("­ƒöö [Auth Event]:", event, session?.user?.email);
      ...
      if (event === 'PASSWORD_RECOVERY') {
            ...
            <div id="recoveryCard" style="background:var(--bg-card); padding:2.5rem; border-radius:20px; border:1px solid rgba(var(--primary-rgb),0.3); width:90%; max-width:420px; text-align:center; box-shadow:0 0 40px rgba(0,0,0,0.6), 0 0 20px rgba(var(--primary-rgb),0.2); transform:translateY(20px); transition:transform 0.4s cubic-bezier(0.34, 1.56, 0.64, 1);">
              ...
              <div style="text-align:left; margin-bottom:1rem;">
                ...
                <div style="position:relative;">
                  <input type="password" id="recoveryNewPassword" autocomplete="new-password" placeholder="M├¡nimo 6 caracteres" style="width:100%; padding:14px; padding-right:45px; background:var(--bg-surface); border:1px solid var(--border); border-radius:10px; color:var(--text-main); outline:none; font-size:1rem; transition:all 0.3s ease; box-sizing:border-box;" />
              ...
              <button id="recoveryBtn" class="btn btn-primary" style="width:100%; padding:14px; font-size:1.1rem; border-radius:10px; font-weight:bold; letter-spacing:1px; text-transform:uppercase; transition:all 0.3s ease;">­ƒÆ¥ Redefinir Senha</button>
            </div>
          </div>
        `;
        ...
        document.getElementById('recoveryBtn').addEventListener('click', async () => {
          ...
          if (!newPassword || newPassword.length < 6) {
            errorDiv.innerHTML = "ÔØî A senha deve ter pelo menos 6 caracteres.";
            errorDiv.style.display = "block";
            return;
          }
          if (newPassword !== confirmPassword) {
            errorDiv.innerHTML = "ÔØî As senhas n├úo coincidem.";
            errorDiv.style.display = "block";
            return;
          }
          ...
          if (error) {
            ...
            errorDiv.innerHTML = "ÔØî Erro: " + msg;
            errorDiv.style.display = "block";
            btn.innerHTML = "­ƒÆ¥ Redefinir Senha";
            btn.disabled = false;
          } else {
            sessionStorage.removeItem('is_recovering_password');
            // Deslogar imediatamente para for├ºar o login manual e impedir que "entre na conta sozinho"
            await supaClient.auth.signOut();
            ...
            setTimeout(() => {
              card.innerHTML = `
                 ...
                 <p style="color:var(--text-muted); margin-bottom:2rem; font-size:1rem; line-height:1.6;">Sua senha foi redefinida com perfei├º├úo. Por seguran├ºa, voc├¬ foi desconectado. Fa├ºa o login com sua nova senha.</p>
      ...
      if (previousSessionId && currentSessionId && previousSessionId !== currentSessionId) {
        console.log("­ƒöä Usu├írio alterado. Atualizando dados locais...");
        try {
          // Limpar chaves gen├®ricas para evitar vazamento
...
// 3. L├│gica de Login e Toggles de Senha
  ...
  forgotPasswordLink.addEventListener("click", async (e) => {
    ...
    try {
      ...
      if (error) {
        ...
        if (successDiv) {
          successDiv.innerHTML = "Ô£à <strong>E-mail de redefini├º├úo enviado!</strong><br/>Verifique sua caixa de entrada.<br/><br/><span style='color:var(--warning); font-size:0.85rem;'>ÔÜá´©Å <strong>Aten├º├úo:</strong> N├úo encontrou? Verifique sua pasta de <strong>Spam ou Lixo Eletr├┤nico</strong>.</span>";
...
if (loginForm) {
  loginForm.addEventListener("submit", async (e) => {
    e.preventDefault();
    // Limpar qualquer flag de recupera├º├úo de senha pendente
    sessionStorage.removeItem('is_recovering_password');
    ...
    // Valida├º├úo b├ísica de formato de e-mail
    const emailRegex = /^[^\s@]+@[^\s@]+\.[^\s@]+$/;
    if (!emailRegex.test(email)) {
      errorDiv.textContent = "ÔØî E-mail inv├ílido ou mal formatado.";
      errorDiv.style.display = "block";
      return;
    }
    ...
    try {
      ...
      if (data.user && !data.user.email_confirmed_at) {
        errorDiv.textContent = "ÔÜá´©Å Confirme seu email antes de entrar.";
        errorDiv.style.display = "block";
        submitBtn.disabled = false;
...
if (registerForm) {
  registerForm.addEventListener("submit", async (e) => {
    ...
    const emailRegex = /^[^\s@]+@[^\s@]+\.[^\s@]+$/;
    if (!emailRegex.test(email)) {
      errorDiv.textContent = "ÔØî Digite um e-mail v├ílido (ex: seu@email.com).";
      errorDiv.style.display = "block";
      return;
    }

    if (password !== confirmPassword) {
      errorDiv.textContent = "As senhas n├úo coincidem.";
      errorDiv.style.display = "block";
      return;
    }
    ...
    if (error) {
      ...
      if (msg.toLowerCase().includes("rate limit") || msg.toLowerCase().includes("too many requests")) {
        msg = "ÔÜá´©Å Limite de tentativas atingido. Por favor, aguarde alguns minutos.";
      } else if (msg.toLowerCase().includes("security purposes")) {
        msg = "ÔÜá´©Å Voc├¬ j├í solicitou um c├│digo para este e-mail. Aguarde 1 minuto para tentar novamente.";
      } else if (msg.toLowerCase().includes("confirmation email") || msg.toLowerCase().includes("email send failed")) {
        msg = "ÔÜá´©Å Erro no servidor de e-mail (SMTP). Por favor, verifique as configura├º├Áes de SMTP no painel do Supabase.";
      } else if (msg.toLowerCase().includes("already registered") || msg.toLowerCase().includes("already exists") || error.status === 422) {
        msg = "ÔØî Algu├®m j├í est├í usando essa conta. Tente fazer login ou use outro e-mail.";
      }
      ...
      // Registro realmente novo - Esconder form e mostrar container de verifica├º├úo
      ...
      if (otpSection) {
        ...
        if (resendSection) {
          ...
          // Pr├®-preencher o campo de reenvio com o e-mail j├í usado
        ...
        if (emailDisplay) {
          emailDisplay.innerHTML = `Quase l├í! Enviamos um c├│digo de seguran├ºa de 6 d├¡gitos para:<br/><strong style="color:var(--primary); font-size:1rem;">${email}</strong><br/><br/><span style="color:var(--warning); font-size:0.9rem;">ÔÜá´©Å <strong>Aten├º├úo:</strong> O e-mail pode demorar 1 minuto para chegar. Se n├úo encontrar, verifique sua caixa de <strong>Spam ou Lixo Eletr├┤nico</strong>.</span>`;
        }
      } else {
        console.error("ÔØî otpSection N├âO encontrado no DOM!");
      }
...
if (resendBtn) {
  ...
  resendBtn.addEventListener("click", async () => {
    ...
    if (!emailToResend) {
      alert("Digite o e-mail para reenviar o c├│digo.");
      return;
    }
    resendBtn.disabled = true;
    resendBtn.textContent = "­ƒô¿ Enviando...";
    const client = window.supabaseClient;
    if (!client) {
      resendBtn.textContent = "ÔØî Erro ao reenviar. Tente novamente.";
      setTimeout(() => { resendBtn.textContent = "­ƒôº Reenviar C├│digo"; resendBtn.disabled = false; }, 3000);
      return;
    }
    ...
    if (error) {
      ...
      resendBtn.textContent = "ÔØî " + msg;
      setTimeout(() => { resendBtn.textContent = "­ƒôº Reenviar C├│digo"; resendBtn.disabled = false; }, 5000);
    } else {
      resendCooldown = true;
      let seconds = 60;
      resendBtn.textContent = `Ô£à C├│digo reenviado! Aguarde ${seconds}s`;
      const interval = setInterval(() => {
        seconds--;
        resendBtn.textContent = `ÔÅ│ Aguarde ${seconds}s para reenviar`;
        if (seconds <= 0) {
          clearInterval(interval);
          resendBtn.disabled = false;
          resendBtn.textContent = "­ƒôº Reenviar C├│digo";
...
if (verifyOtpBtn) {
  // Se j├í temos um e-mail salvo, podemos mostrar a se├º├úo de OTP direto
  if (lastEmailRegistered && document.getElementById("otpSection")) {
    // document.getElementById("otpSection").style.display = "block"; 
    // (Opcional: descomente se quiser que apare├ºa ao atualizar a p├ígina)
  }
  ...
  verifyOtpBtn.addEventListener("click", async () => {
    ...
    if (!email) {
      otpError.textContent = "Erro: E-mail n├úo identificado.";
      otpError.style.display = "block";
      return;
    }

    if (!token || token.length < 6) {
      otpError.textContent = "Digite o c├│digo completo recebido por e-mail.";
      otpError.style.display = "block";
      return;
    }

    verifyOtpBtn.disabled = true;
    verifyOtpBtn.textContent = "­ƒöî Verificando...";
    otpError.style.display = "none";

    console.log("­ƒöì [DEBUG] Iniciando Verifica├º├úo OTP:");
    ...
    try {
      ...
      if (res.error) {
        console.warn("   ÔÜá´©Å Falha 'signup':", res.error.message);
        // 2. Tenta tipo 'email'
      ...
      if (res.error) {
        console.warn("   ÔÜá´©Å Falha 'email':", res.error.message);
      ...
      if (error) {
        console.error("ÔØî ERRO FINAL:", error);
        let userMsg = "C├│digo inv├ílido.";
        if (error.message.includes("expired")) userMsg = "O c├│digo expirou ou ├® antigo.";
        else if (error.message.includes("not found")) userMsg = "E-mail n├úo encontrado.";

        otpError.textContent = `ÔØî ${userMsg} (${error.message})`;
        otpError.style.display = "block";
        verifyOtpBtn.disabled = false;
        verifyOtpBtn.textContent = "Ô£à Verificar e Entrar";
      } else {
        sessionStorage.removeItem("lastEmailRegistered");
        // For├ºar o Supabase a atualizar a sess├úo internamente antes do redirect
      ...
      otpError.textContent = "Erro inesperado na verifica├º├úo.";
      otpError.style.display = "block";
      verifyOtpBtn.disabled = false;
    }
  });
...
window.deleteUserAccount = async function () {
  if (!supaClient) return { error: { message: 'Supabase n├úo inicializado.' } };

  try {
    const { data: { user } } = await supaClient.auth.getUser();
    if (!user) return { error: { message: 'Usu├írio n├úo autenticado.' } };

    // Chama a fun├º├úo de seguran├ºa (Postgres RPC) no Supabase
...
window.loadBanners = async function () {
  ...
  if (window.supabaseClient) {
    try {
      console.log("­ƒø░´©Å Sincronizando com Storage do Supabase...");
      const { data: supaBanners, error } = await window.supabaseClient
        .from("store_banners")
        .select("id, image_url");

      if (error) {
        console.error("ÔØî Erro ao buscar banners do Supabase:", error);
        return;
      }

      console.log(
        `­ƒôª Banners carregados do Supabase: ${supaBanners?.length || 0} itens`,
      );
      console.log("­ƒöì Raw data do Supabase:", supaBanners);
      ...
      if (supaBanners && supaBanners.length > 0) {
        supaBanners.forEach((b, index) => {
          ...
          console.log(`­ƒôØ [${index}] ID="${cleanId}" | hasURL=${hasUrl}`);
          ...
          if (b.image_url) {
            ...
            if (fileName.includes("cosmos")) {
              ...
              console.log(`Ô£à MAPEADO banner_cosmos via arquivo: ${fileName}`);
            }
            ...
            if (cleanId.includes("cosmos")) {
              window.BANNER_MAP["banner_cosmos"] = b.image_url;
              console.log(`Ô£à MAPEADO banner_cosmos via ID: ${cleanId}`);
            }
            ...
            // Se o usu├írio subir com extens├úo, mapeia tamb├®m
        ...
        if (window.BANNER_MAP["banner_cosmos"]) {
          console.log("Ô£à banner_cosmos est├í dispon├¡vel!");
        } else {
          ...
          console.log("­ƒøá´©Å banner_cosmos mapeado via HARD-FIX");
        }
      } else {
        console.warn("ÔÜá´©Å Nenhum banner retornado do Supabase");
      }
      updateNavbarCosmetics();
    } catch (err) {
      console.error("ÔØî Erro Banners:", err);
      console.error(err.stack);
    }
  } else {
    console.warn("ÔÜá´©Å supabaseClient n├úo dispon├¡vel");
  }
};
...
window.updateNavbarCosmetics = function () {
  ...
  // ­ƒÆí Se BANNER_MAP estiver vazio, tenta carregar banners primeiro
  ...
  // ­ƒÆí Dados do Banco (Preferencial)
  let sData = null;
  let userId = null;
  if (window.supabaseClient) {
    // Tenta pegar o user ID de forma s├¡ncrona se poss├¡vel ou via cache
  ...
  if (window.DB && window.DB._store && window.DB._store.profile) {
    ...
    // ­ƒÆí Tenta carregar da chave ISOLADA do usu├írio
    try {
  ...
  // Chaves para cosm├®ticos - agora SEMPRE priorizando o banco ou o userId
  ...
  // ­ƒîê TEMA CROM├üTICO ÔÇö restaurar se estava equipado
  if (hasTemaCromatico && window.setTheme) {
    window.setTheme("theme-cromatico");
  }

  // ­ƒô© L├ôGICA DE BANNER DA NAVBAR
  if (bannerBg) {
    if (savedBanner !== "none" && window.BANNER_MAP) {
      ...
      if (url) {
        ...
        console.warn(
          `ÔÜá´©Å Link do banner '${savedBanner}' n├úo encontrado no BANNER_MAP.`,
        );
    ...
    // S├│ tenta novamente em p├íginas que realmente t├¬m navbar de usu├írio (n├úo login/registro)
    const isAuthPage = window.location.pathname.includes("login.html") || window.location.pathname.includes("registro.html");
    if (!isAuthPage) {
  ...
  // ÔÇö Sidebar avatar (abaixo do hist├│rico no perfil) ÔÇö
  ...
  if (displayAvatarBox) {
    ...
    if (sbCrown) {
      ...
      if (savedCrown) {
        const crownIcon = getCosmetic("crownIcon", "­ƒææ");
  ...
  // ­ƒÅá APLICAR EM P├üGINAS ESPEC├ìFICAS (PERFIL / HIST├ôRICO)
  if (window.location.href.includes("perfil.html")) {
    ...
    if (sidebar) {
      ...
      // Remover overlay antigo se existir (abordagem antiga com z-index:-1 que n├úo funcionava)
      ...
      if (bannerUrl) {
        ...
        // Isso evita o problema de z-index onde o overlay ficava atr├ís do fundo da p├ígina
        ...
        // Sem banner: restaurar background padr├úo do CSS
...
// ­ƒÜÇ Rastreamento de Presen├ºa (Online Status)
window.startPresenceHeartbeat = function() {
  ...
  const updateStatus = async () => {
    try {
      ...
      if (error) {
        console.warn("ÔÜá´©Å N├úo foi poss├¡vel atualizar status online. Verifique se a coluna 'last_seen' existe na tabela 'profiles'.", error.message);
      }
    } catch (e) {
      console.error("Erro no heartbeat de presen├ºa:", e);
    }
  };
...
// ­ƒÜÇ Carregar banners automaticamente quando DOM estiver pronto
document.addEventListener("DOMContentLoaded", () => {
  console.log("­ƒÜÇ DOM pronto - Iniciando carregamento de banners...");
  ...
  const tryLoadBanners = (attempt = 1) => {
    if (window.supabaseClient && typeof window.loadBanners === "function") {
      ...
      console.warn("ÔÜá´©Å Supabase n├úo dispon├¡vel ap├│s 10 tentativas");
    }
  };

  tryLoadBanners();

  // Mostrar email no OTP section se j├í estivermos no meio de um registro
  if (lastEmailRegistered) {
    const otpSection = document.getElementById("otpSection");
    if (otpSection) {
      const emailMsg = otpSection.querySelector("p");
      if (emailMsg) {
        emailMsg.innerHTML = `<strong>­ƒöæ Digite o c├│digo de 6 d├¡gitos</strong><br/><small style="color:var(--primary); opacity:0.8;">Enviado para: ${lastEmailRegistered}</small>`;
      }
    }
  }
  ...
  if (window.location.hash) {
    ...
    if (hashParams.get("error_code") === "otp_expired" || hashParams.get("error_description")) {
      console.warn("Autentica├º├úo Hash:", hashParams.get("error_description"));
...
window.checkAuthStatus = async function () {
  ...
  // Ocultar bot├Áes de login na home se estiver logado
  ...
  if (nav) {
    ...
    if (session) {
      ...
      // Se estamos no perfil e n├úo temos perfil no banco, n├úo expulsar imediatamente
      if (!profile && currentPage === "perfil.html") {
        console.warn("Perfil ainda n├úo criado no banco. Aguardando sincroniza├º├úo...");
      }

      if (!window.BANNER_MAP) await loadBanners();
      else updateNavbarCosmetics();

      // Iniciar batimento card├¡aco de presen├ºa
      if (typeof window.startPresenceHeartbeat === "function") {
        window.startPresenceHeartbeat();
      }
    } else {
      authContainer.innerHTML = `<a href="login.html" class="btn btn-primary btn-sm">­ƒæñ Entrar</a>`;
      nav.appendChild(authContainer);
    }
  }
...
// ­ƒÜÇ Preencher links da Navbar automaticamente
function populateNavbarLinks() {
  const navLinks = document.getElementById("navLinks");
  if (!navLinks) return;

  // Define os links padr├úo do site
  const links = [
    { name: "­ƒÅá In├¡cio", url: "index.html" },
    { name: "Ôø®´©Å Animes", url: "animes.html" },
    { name: "­ƒô║ Desenhos", url: "catalogo-desenhos.html" },
    { name: "­ƒÄ¼ Filmes", url: "filmes.html" },
    { name: "­ƒôØ Cad. Desenhos", url: "cadastro.html" },
    { name: "­ƒôØ Cad. Animes", url: "cadastro-animes.html" },
    { name: "­ƒôØ Cad. Filmes", url: "cadastro-filmes.html" },
    { name: "­ƒôØ Cad. YouTube", url: "cadastro-youtube.html" },
    { name: "­ƒôÜ Mang├ís", url: "mangas.html" },
    { name: "ÔûÂ´©Å YouTube", url: "youtube.html" },
    { name: "­ƒÄî SenseiMod Store", url: "loja.html" },
    { name: "­ƒñû Open AnIme", url: "open-anime.html" },
    { name: "­ƒÆû Agradecimento", url: "agradecimento.html" }
  ];
...
async function signInWithGithub() {
  ...
  if (error) {
    ...
    if (typeof showToast === "function") showToast("ÔØî Erro ao entrar com GitHub", "danger");
  }
}
...
async function signInWithGoogle() {
  ...
  if (error) {
    ...
    if (typeof showToast === "function") showToast("ÔØî Erro ao entrar com Google", "danger");
  }
}
...
// Adicionar listeners para os bot├Áes social se existirem
...
// ÔöÇÔöÇÔöÇ Enter global para inputs ÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇ
// Permite concluir qualquer a├º├úo do site pressionando Enter em um campo de texto,
// exceto no campo de exclus├úo de conta (para evitar acidentes).
document.addEventListener("keydown", (e) => {
  ...
  // Nunca ativar Enter em campos de exclus├úo de conta
  const dangerIds = ["deleteConfirmEmail", "deleteConfirmPassword"];
  if (dangerIds.includes(el.id)) return;

  // 1. Se j├í est├í dentro de um <form> com bot├úo submit, deixar o comportamento nativo
  ...
  //    Procura o bot├úo de a├º├úo prim├írio mais pr├│ximo na mesma se├º├úo/container
  const EXCLUDED_BTN_IDS = ["deleteAccountBtn", "confirmDeleteBtn"];

  // Mapa de input ÔåÆ bot├úo de a├º├úo
  // op
  const inputToBtnMap = {
  ...
  // 3. Fallback: procura o primeiro bot├úo prim├írio vis├¡vel no mesmo container pai
...
// ==========================================
// 6. L├│gica de Login e Registro por Discord
// ==========================================
