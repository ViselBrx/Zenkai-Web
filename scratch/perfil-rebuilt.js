...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    function syncLiveProfileUI(overrides = {}) {
      ...
      const nextTitle = String(overrides.customTitle ?? localStorage.getItem('animehouse_customTitle') ?? '').trim();
      const showCrown = typeof overrides.showCrown === 'boolean'
        ? overrides.showCrown
      ...
      if (displayCustomTitleEl) displayCustomTitleEl.textContent = nextTitle;
      if (sidebarCrownEl) sidebarCrownEl.style.display = showCrown ? 'block' : 'none';
